...
func init() {
	scraperutil.RegisterValidator("fc2", func(a any) error {
	...
	scraperutil.RegisterScraperConfig("fc2", func(a any) any {
		return nil
	})
	// TASK 5: Register ConfigFactory for UnmarshalYAML
	scraperutil.RegisterConfigFactory("fc2", func() any {
		return &FC2Config{}
	})
	// TASK 3: Register flatten function for registry-based type conversion
	scraperutil.RegisterFlattenFunc("fc2", func(cfg any) any {
		...
		// Use type assertion to access FC2-specific fields
		if fc2Cfg, ok := cfg.(*FC2Config); ok {
			return &config.ScraperSettings{
				...
				FlareSolverr:  fc2Cfg.FlareSolverr,
			}
		}
		return nil
	})
...
func (c *FC2Config) ValidateConfig(sc *config.ScraperSettings) error {
	...
	if sc.FlareSolverr.Enabled {
		if sc.FlareSolverr.URL == "" {
			return fmt.Errorf("fc2.flaresolverr.url is required when flaresolverr is enabled")
		}
		if sc.FlareSolverr.Timeout < 1 || sc.FlareSolverr.Timeout > 300 {
			return fmt.Errorf("fc2.flaresolverr.timeout must be between 1 and 300")
		}
		if sc.FlareSolverr.MaxRetries < 0 || sc.FlareSolverr.MaxRetries > 10 {
			return fmt.Errorf("fc2.flaresolverr.max_retries must be between 0 and 10")
		}
		if sc.FlareSolverr.SessionTTL < 60 || sc.FlareSolverr.SessionTTL > 3600 {
			return fmt.Errorf("fc2.flaresolverr.session_ttl must be between 60 and 3600")
		}
	}
	return nil
}
