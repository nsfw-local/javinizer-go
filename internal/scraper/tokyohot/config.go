...
func init() {
	...
	scraperutil.RegisterFlattenFunc("tokyohot", func(cfg any) any {
		...
		// Use type assertion to access TokyoHot-specific fields
		if tokyohotCfg, ok := cfg.(*TokyoHotConfig); ok {
			return &config.ScraperSettings{
				...
				FlareSolverr:  tokyohotCfg.FlareSolverr,
			}
		}
		return nil
	})
...
func (c *TokyoHotConfig) ValidateConfig(sc *config.ScraperSettings) error {
	...
	if sc.FlareSolverr.Enabled {
		if sc.FlareSolverr.URL == "" {
			return fmt.Errorf("tokyohot.flaresolverr.url is required when flaresolverr is enabled")
		}
		if sc.FlareSolverr.Timeout < 1 || sc.FlareSolverr.Timeout > 300 {
			return fmt.Errorf("tokyohot.flaresolverr.timeout must be between 1 and 300")
		}
		if sc.FlareSolverr.MaxRetries < 0 || sc.FlareSolverr.MaxRetries > 10 {
			return fmt.Errorf("tokyohot.flaresolverr.max_retries must be between 0 and 10")
		}
		if sc.FlareSolverr.SessionTTL < 60 || sc.FlareSolverr.SessionTTL > 3600 {
			return fmt.Errorf("tokyohot.flaresolverr.session_ttl must be between 60 and 3600")
		}
	}
	return nil
}
