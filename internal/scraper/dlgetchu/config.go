...
func init() {
	...
	scraperutil.RegisterFlattenFunc("dlgetchu", func(cfg any) any {
		...
		// Use type assertion to access DLGetchu-specific fields
		if dlgetchuCfg, ok := cfg.(*DLGetchuConfig); ok {
			return &config.ScraperSettings{
				...
				FlareSolverr:  dlgetchuCfg.FlareSolverr,
			}
		}
		return nil
	})
...
func (c *DLGetchuConfig) ValidateConfig(sc *config.ScraperSettings) error {
	...
	if sc.FlareSolverr.Enabled {
		if sc.FlareSolverr.URL == "" {
			return fmt.Errorf("dlgetchu.flaresolverr.url is required when flaresolverr is enabled")
		}
		if sc.FlareSolverr.Timeout < 1 || sc.FlareSolverr.Timeout > 300 {
			return fmt.Errorf("dlgetchu.flaresolverr.timeout must be between 1 and 300")
		}
		if sc.FlareSolverr.MaxRetries < 0 || sc.FlareSolverr.MaxRetries > 10 {
			return fmt.Errorf("dlgetchu.flaresolverr.max_retries must be between 0 and 10")
		}
		if sc.FlareSolverr.SessionTTL < 60 || sc.FlareSolverr.SessionTTL > 3600 {
			return fmt.Errorf("dlgetchu.flaresolverr.session_ttl must be between 60 and 3600")
		}
	}
	return nil
}
